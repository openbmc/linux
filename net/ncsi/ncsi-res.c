/*
 * Copyright Benjamin Herrenschmidt & Gavin Shan, IBM Corporation 2015.
 *
...
static void ncsi_release_package(struct ncsi_package *np)
{
...
static void ncsi_dev_init(struct ncsi_dev_priv *ndp)
{
	struct ncsi_dev *nd = &ndp->ndp_ndev;
	struct net_device *dev = ndp->ndp_ndev.nd_dev;
	struct ncsi_package *np;
	struct ncsi_channel *nc;
	...
	nca.nca_nlh = NULL;
	switch (nd->nd_state) {
	case ncsi_dev_state_init:
		nd->nd_state = ncsi_dev_state_init_deselect;
		/* Fall through */
	case ncsi_dev_state_init_deselect:
		...
		nd->nd_state = ncsi_dev_state_init_package;
		break;
	case ncsi_dev_state_init_package:
		...
		nd->nd_state = ncsi_dev_state_init_channel;
		break;
	case ncsi_dev_state_init_channel:
		...
		if (!ndp->ndp_active_package) {
			...
			if (list_is_last(&ndp->ndp_active_package->np_node,
					 &ndp->ndp_packages)) {
				nd->nd_state = ncsi_dev_state_init_active;
	...
	case ncsi_dev_state_init_sp:
		...
		nd->nd_state = ncsi_dev_state_init_cis;
		break;
	case ncsi_dev_state_init_cis:
		...
		nd->nd_state = ncsi_dev_state_init_gvi;
		break;
	case ncsi_dev_state_init_gvi:
	case ncsi_dev_state_init_gc:
		...
		if (nd->nd_state == ncsi_dev_state_init_gvi)
		...
		if (nd->nd_state == ncsi_dev_state_init_gvi)
			nd->nd_state = ncsi_dev_state_init_gc;
		else
			nd->nd_state = ncsi_dev_state_init_dp;
		break;
	case ncsi_dev_state_init_dp:
		atomic_set(&ndp->ndp_pending_reqs, 1);
		...
		nd->nd_state = ncsi_dev_state_init_channel;
		break;
	case ncsi_dev_state_init_active:
choose_active_channel:
		 ...
		 */
		nd->nd_state = ncsi_dev_state_init_sma;
		ndp->ndp_active_package = NULL;
		ncsi_select_active_channel(ndp);
		if (!ndp->ndp_active_channel)
			goto error;
		/* Fall through */
	case ncsi_dev_state_init_sma:
	case ncsi_dev_state_init_ebf:
	case ncsi_dev_state_init_ecnt:
	case ncsi_dev_state_init_ec:
	case ncsi_dev_state_init_gls:
		atomic_set(&ndp->ndp_pending_reqs, 1);
		 ...
		 * 0.
		 */
		if (nd->nd_state == ncsi_dev_state_init_sma) {
			nca.nca_type = NCSI_PKT_CMD_SMA;
			for (index = 0; index < 6; index++)
				nca.nca_bytes[index] = dev->dev_addr[index];
			nca.nca_bytes[6] = 0x1;
			nca.nca_bytes[7] = 0x1;
			nd->nd_state = ncsi_dev_state_init_ebf;
		} else if (nd->nd_state == ncsi_dev_state_init_ebf) {
			nca.nca_type = NCSI_PKT_CMD_EBF;
			nca.nca_dwords[0] = nc->nc_caps[NCSI_CAP_BC].ncc_cap;
			nd->nd_state = ncsi_dev_state_init_ecnt;
		} else if (nd->nd_state == ncsi_dev_state_init_ecnt) {
			nca.nca_type = NCSI_PKT_CMD_ECNT;
			nd->nd_state = ncsi_dev_state_init_ec;
		} else if (nd->nd_state == ncsi_dev_state_init_ec) {
			nca.nca_type = NCSI_PKT_CMD_EC;
			nd->nd_state = ncsi_dev_state_init_gls;
		} else if (nd->nd_state == ncsi_dev_state_init_gls) {
			nca.nca_type = NCSI_PKT_CMD_GLS;
			nd->nd_state = ncsi_dev_state_init_done;
		}
...
EXPORT_SYMBOL_GPL(ncsi_find_dev);
...
int ncsi_start_dev(struct ncsi_dev *nd)
{
	...
	nd->nd_state = ncsi_dev_state_init;
	schedule_work(&ndp->ndp_work);
