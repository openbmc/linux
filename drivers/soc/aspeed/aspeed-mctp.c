...
static ssize_t aspeed_mctp_netdev_tx(struct sk_buff *skb, struct net_device *ndev)
{
	...
	tx_header = tx_packet->data.hdr;

	skb_copy_bits(skb, 0, &tx_packet->data, skb->len);
	tx_packet->size = ((skb->len + 3) / 4) * 4;

	data = (u8 *) &tx_packet->data;
	hdr_dw = (u32 *) tx_packet->data.hdr;
	hdr = (u8 *) tx_packet->data.hdr;
	...
	packet_data_sz_dw = tx_packet->size / sizeof(u32) - sizeof(tx_packet->data.hdr) / sizeof(u32);
	if (packet_data_sz_dw != pci_data_len_dw) {
...
static int aspeed_mctp_probe(struct platform_device *pdev)
{
	...
	if (id < 0) {
		dev_err(priv->dev, "Failed to device node id\n");
		goto out_dma;
	}
